    ...
    <>
      ...
      <header className="mx-auto max-w-[1160px] px-6 md:px-8">
        <div className="flex flex-col items-center py-[clamp(56px,10vh,104px)] text-center">
          ...
          <p
            className="fade-up mt-3 text-[13px] text-muted"
            style={{ animationDelay: "0.22s" }}
          >
            {RECORD.wrongBindings} wrong bindings {RECORD.scope}.
          </p>
        ...
        <div className="mx-auto grid max-w-[1160px] grid-cols-1 gap-4 px-6 py-9 sm:grid-cols-2 md:px-8 lg:grid-cols-4">
          {[
            ...
            <div key={t} className="flex items-start gap-3">
              ...
              <span className="text-[13.5px] leading-snug text-body">{t}</span>
            </div>
          ))}
        </div>
      </section>
          ...
          <Frame
            ...
            caption={
              <>
                ...
                <ExtLink href={LINKS.erpLedger} className="underline">
                  public
                </ExtLink>
                .
              </>
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2">
          {BOUNDARIES.map((b) => (
            <div key={b.k} className="card p-7">
              <h3
                className="mb-3 mt-0 font-normal"
                style={{ fontFamily: "var(--font-mono)", fontSize: "0.8rem", color: "var(--color-sienna)" }}
              >
                {b.k}
              </h3>
              <p className="m-0 text-[14.5px] leading-relaxed text-body">{b.v}</p>
            </div>
          ))}
        </div>
      ...
      <Section id="evidence" tint>
        ...
        <p className="mt-8 max-w-[76ch] text-[15px] leading-relaxed text-body">
        ...
        <div className="mt-10 grid grid-cols-1 gap-px overflow-hidden rounded-xl sm:grid-cols-2 lg:grid-cols-4" style={{ background: "var(--color-border)", border: "1px solid var(--color-border)" }}>
          {[
            ...
            },
            {
              n: "04",
              t: "Serve",
              b: "Your own build generates the manifest from your source, and a small runtime inside your app serves the tools to agents over MCP. The same model also emits an OpenAPI document.",
            },
          ].map((s) => (
